...
Переменные для управления поведением hasher (config-hsh.mk):
...
Для каждого конфигурационного файла определяется набор констант, не
предназанченых для переопределения. Они описывают структуру профиля и его
положение на файловой системе.
...
CURDIR        - Текущий каталог. Там где происходит выполенение.

PREVDIR       - Каталог с предыдущим конфигурационным файлом.

TOPDIR        - Коренвой каталог всего профиля. Переменная определена если в
                корне профиля создан каталог `.mki', иначе эта переменная равна
                CURDIR.
