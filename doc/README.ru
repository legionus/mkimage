...
mkimage состоит из нескольких частей:

1. Определение переменных окружения, влияющих на работу утилит и правил
(config.mk).

2. Описание сборки стадии (targets.mk). Этот файл содержит описание целей,
используемых при сборки стадий;

3. Готовые скрипты для реализации отдельных стадий сборки (tools/*). Эти скрипты
описаны в tools.mk для использования в конфигурационном файле.
...
действие. Ими могут являться как создание загрузочного образа, так и
копирование в рабочий чрут каких-либо файлов. Вы можете как пользоваться 
предопределёнными целями, так и писать свои.
...
Параметры PROPAGATOR_MAR_MODULES, PROPAGATOR_INITFS и PROPAGATOR_VERSION используются
...
BOOT_TYPE              - Описывает, каким образом будет загружаться образ. Доступные
                         значения: 'isolinux' и 'pxelinux'. Возможно указать
...
понадобиться совершенно разные программы. Нельзя требовать наличие их в
системе, где происходит сборка.
...
[1] iso-disk                 <- SUBDIRS = installer RPMS.basesystem RPMS.extra
  \ [1.1] installer          <- SUBDIRS = stage1 stage2 stage3 stage4
     \ [1.1.1] stage1
     \ [1.1.2] stage2
     \ [1.1.3] stage3        <- SUBDIRS = stage3.1
        \ [1.1.3.1] stage3.1
     \ [1.1.4] stage4
  \ [1.2] RPMS.basesystem
  \ [1.3] RPMS.extra
...
Чтобы указать от каких директорий зависит стадия используется переменная
SUBDIRS. В ней перечислены имена каталогов от которых зависит данная стадия.
