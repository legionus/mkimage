...
distclean-current  - Полностью отчищает образ от служебных файлов mkimage и
...
GLOBAL_VERBOSE        - Включает отладочную информацию локально и глобально (
                        для всех нижестоящих образов) соотвтетственно.
TARGET,
...
MKI_SCRIPTDIR         - Определяает название директории, содержащей скрипты для
                        исполнения в инструментальном чруте.
			По умолчанию: $(CURDIR)/scripts.d

MKI_IMAGE_SCRIPTDIR   - Определяает название директории, содержащей скрипты для
