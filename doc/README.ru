...
1. Описание сборки стадии (rules.mk). Этот файл содержит описание целей,
используемых при сборки стадий и определение переменных окружения, влияющих
на работу утилит.

2. Готовые скрипты для для реализации отдельных стадий сборки (tools/*).
...
CHROOT_PACKAGES_REGEXP - Переменная имеет тот же смысл что CHROOT_PACKAGES, но
                        за исключением того что тут перечиляются не только имена
			пакетов, но и регулярные выражения для фильтрации списка
			пакетов.
...
IMAGE_PACKAGES_REGEXP - Переменная имеет тот же смысл что IMAGE_PACKAGES.
                        Содержимое этой переменной аналогично CHROOT_PACKAGES_REGEXP.
...
BOOT_TYPE             - Описывает каким образом будет загружаться образ. Доступные
			...
			конфигурации для перечисленных методов.
...
BOOT_VOLI, BOOT_VOLS
		      ...
		      указываются .mkisofsrc: APPI, COPY, ABST, BIBL, PREP,
		      PUBL, SYSI, VOLI, VOLS.
		      См. mkisofs(8).
...
Любую стадию можно условно разделить на несколько частей:

1. Конфигурационный файл (Makefile);
2. Входные данные;
3. Сборочная область;
4. Результат стадии.
...
С конфигурационным файлом всё понятно, о нём рассказывать нечего. С входными
данными тоже, они описаны в конфигурационном файле и используются на стадии
сборки.
...
понадобиться совершенно разные программы. Нельзя требовать наличие их в системе
где происходит сборка.
...
обработки, потому что результатом может быть как один файл, так и несколько
файлов или каталогов. Так как результат заранее не известен, эта директория
очищается перед каждой операцией записи. Это нужно учитывать и не оставлять в
ней ничего ценного между сборками. Это поведение можно изменить переменной
CLEANUP_OUTDIR, но это потенциально опасно т.к. результаты будут накладываться
друг на друга.

Кэш. Кэш сссука полезный. В кэш, по возможности, заносятся данные о каждой
...
удасться т.к. не известно что произошло с файлами в чруте потом.
...
Каждая из стадий обрабатывается одинаково, начиная с самого большого уровня
вложенности.
