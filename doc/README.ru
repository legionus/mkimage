...
GLOBAL_PACKAGES_EXPAND_METHOD
                        В mkimage есть возможность дополнять списки
                        CHROOT_PACKAGES/IMAGE_PACKAGES перед выполнением замыкания.
			Эта опция включает/выключает такую обработку. Возможные
			значения этого пвраметра: "none" и "regexp".
			Значение "regexp" означает что все строки, содержащие
			специальные символы, расцениваются как регулярные выражения.
			Если строка это регулярное выражение, то оно применяется
			к списку пакетов в репозитории. Результат будет добавлен
			к списку CHROOT_PACKAGES/IMAGE_PACKAGES. Остальные строки
			в этих списках остаются без изменения.
			Значение "none" означает, что никакого дополнения проводить
			не нужно.
			По умолчанию: none
