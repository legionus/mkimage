...
copy-grub-efi      - Копирует в рабочий чрут файлы, необходимые для GRUB EFI.
...
copy-grubaa64boot  - Копирует в рабочий чрут файлы, необходимые для grub-efi в
                     ...
                     него можно использовать copy-grub-efi.
...
BOOT_TYPE              - Описывает, каким образом будет загружаться образ. Доступные
                         значения: 'isolinux-boot', 'pxelinux', 'syslinux', 'grub-efi',
                         ...
                         момент поддерживаются комбинации 'isolinux-boot grub-efi' и
                         'grub-pc-boot grub-efi'.
...
PACKAGES_REQUIRED_COPY_GRUB_EFI      = dosfstools mtools hardlink
